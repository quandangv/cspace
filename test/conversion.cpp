...
vector<TestSet> conversion_tests{
  ...
  {{1, 1, 1}, {0, 0, 1}, {0, 0, 1}, {.95047, 1, 1.08883}, {1, 0, 0}},
  {{0.5, 0.5, 0.5}, {0, 0, 0.5}, {0, 0, 0.5}, {.2034397, 0.2140411, 0.2330544}, {0.53389, 0, 0}},
  {{1, 0, 0}, {0, 1, 0.5}, {0, 1, 1}, {.412456, .212673, .019334}, {0.5324079, 0.8009249, 0.6720319}},
  {{0.5, 0.25, 1}, {260, 1, 0.625}, {260, 0.75, 1}, {.286912, .154080, .960506}, {0.4618801, 0.6735635, -0.8459172}},
};
...
bool expect_near(const double* output, const double* expected, const double* input, const double* rgb, string from, string to) {
  auto result = std::abs(output[0] - expected[0]) <= tolerance &&
                std::abs(output[1] - expected[1]) <= tolerance &&
                std::abs(output[2] - expected[2]) <= tolerance;
  EXPECT_TRUE(result) << "Conversion from " << from << " to " << to << " is wrong, input: " << to_string3(input) << "\nOutput: " << to_string3(output) << "\nExpected: " << to_string3(expected) << "\nOriginal RGB: " << to_string3(rgb) << endl;
  return result;
}
