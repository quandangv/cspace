...
inline double inv_f(double l) {
  double ll = pow(l, 1/p);
  return 1e4 * pow((c1 - ll) / (c3*ll - c2), 1/n);
}

constexpr double d0 = 1.6295499532821566e-11;
constexpr double b = 1.15;
constexpr double d = 0.66;

void xyz_jzazbz(const double* i, double* o) {
  double xp = b*i[0] - (b-1)*i[2];
  double yp = d*i[1] - (d-1)*i[0];
  double lp = f(0.41478972*xp + 0.579999*yp + 0.0146480*i[2]);
  double mp = f(-0.2015100*xp + 1.120649*yp + 0.0531008*i[2]);
  double sp = f(-0.0166008*xp + 0.264800*yp + 0.6684799*i[2]);
  double iz = 0.5 * (lp + mp);
...
void jzazbz_xyz(const double* i, double* o) {
  ...
  double l = inv_f(iz + 0.255917185920468419580*i[1] + 0.49301692226219780071*i[2]);
  double m = inv_f(iz - 0.021292900622610172173*i[1] + 0.37692228994996004914*i[2]);
  double s = inv_f(iz + 0.021292900622610172173*i[1] - 0.37692228994996004914*i[2]);
